...
LOGGER = get_logger("weather_batch_collector")
WEATHER_BATCH_TOPIC = "weather.raw.batch"
WEATHER_DLQ_TOPIC = "weather.raw.dlq"
OPEN_METEO_ARCHIVE_URL = "https://archive-api.open-meteo.com/v1/archive"
PIPELINE_NAME = "weather_batch_ingestion"
LOCATION_CATALOG = [
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch ingest historical weather data to Kafka.")
    parser.add_argument("--start-date", required=True, help="YYYY-MM-DD")
    parser.add_argument("--end-date", required=True, help="YYYY-MM-DD")
    parser.add_argument("--topic", default=WEATHER_BATCH_TOPIC)
...
def fetch_weather_history(location: dict[str, Any], start_date: str, end_date: str) -> dict[str, Any]:
    params = {
...
def iter_hourly_weather_records(payload: dict[str, Any], city: str) -> list[dict[str, Any]]:
        ...
        records.append(
            {
                ...
                "current": {
                    "time": event_time,
                    "temperature_2m": _safe_array_value(hourly.get("temperature_2m"), index),
...
def main() -> None:
    ...
    batch_id = build_batch_id("weather_batch", args.start_date, args.end_date)

    log_event(
        LOGGER,
        "weather_batch_started",
        topic=args.topic,
    ...
    try:
        for location in locations:
            try:
                payload = fetch_weather_history(location, args.start_date, args.end_date)
            except Exception as exc:
                failed_count += 1
                log_event(LOGGER, "weather_fetch_failed", city=location["city"], error=str(exc))
                _send_dlq_event(
                    producer,
            ...
            for raw_record in iter_hourly_weather_records(payload, city=location["city"]):
                try:
                    normalized = normalize_weather(raw_record, source="open-meteo").dict()
                    dedup_key = (
                        normalized["source"],
                        normalized["city"],
                    ...
                    if dedup_key in seen_keys:
                        ...
                        _send_dlq_event(
                            ...
                            error_message="Duplicate weather record skipped",
                    ...
                    metadata = produce_json_message(
                        ...
                        message=enrich_ingestion_metadata(
                            ...
                            batch_id=batch_id,
                            source_endpoint=OPEN_METEO_ARCHIVE_URL,
                        ),
                    )
                    ...
                    failed_count += 1
                    log_event(LOGGER, "weather_record_failed", city=location["city"], error=str(exc))
                    _send_dlq_event(
                        producer,
                        dlq_topic=args.dlq_topic,
                        batch_id=batch_id,
...
def _send_dlq_event(
    ...
    try:
        ...
        log_event(LOGGER, "weather_dlq_failed", city=city, error=str(exc), original_error=error_message)
