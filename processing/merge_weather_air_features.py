...
def add_targets(df: DataFrame, target_hours: list[int]) -> DataFrame:
    ...
    for hours in target_hours:
        ...
        df = df.withColumn(f"target_wind_U_{hours}h", F.lead("wind_U", hours).over(window_spec))
        df = df.withColumn(f"target_wind_V_{hours}h", F.lead("wind_V", hours).over(window_spec))
...
def main() -> None:
    ...
    try:
        ...
        features = add_targets(features, target_hours=list(range(1, 25)))
