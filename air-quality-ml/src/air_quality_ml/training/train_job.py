...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train MLlib regression model for forecasting")
...
def _configured_feature_columns(dataset, job_settings) -> tuple[list[str], list[str]]:
    feature_cfg = job_settings.training.get("features") or {}
    numeric_cfg = feature_cfg.get("numeric")
    categorical_cfg = feature_cfg.get("categorical")

    if numeric_cfg is None and categorical_cfg is None:
        return get_default_feature_columns(dataset, target_col=job_settings.target_col)

    available = set(dataset.columns)
    excluded_targets = {c for c in dataset.columns if c.startswith("target_")}
    numeric_features = [
        c
        for c in list(numeric_cfg or [])
        if c in available and c not in excluded_targets and c != job_settings.target_col
    ]
    categorical_features = [
        c
        for c in list(categorical_cfg or [])
        if c in available and c not in excluded_targets and c != job_settings.target_col
    ]
    missing_features = [
        c
        for c in list(numeric_cfg or []) + list(categorical_cfg or [])
        if c not in available
    ]
    if missing_features:
        raise ValueError(f"Configured feature columns not found: {missing_features}")
...
def main() -> None:
    ...
    try:
        ...
        numeric_features, categorical_features = _configured_feature_columns(dataset, job_settings)
