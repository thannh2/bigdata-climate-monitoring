...
def main() -> None:
    ...
    input_path_cfg = args.input_path or settings.data.gold_targets_path
    ...
    try:
        ...
        pred_df.write.mode("append").parquet(output_path)
        log_event(logger, "batch_score_written", output_path=output_path)
        ...
        if args.mongo_uri:
            ...
            log_event(logger, "batch_score_mongo_written", db=args.mongo_db, collection=args.mongo_collection)
    finally:
        spark.stop()
